/* vbomesh was taken directly from GLCookbook but was modified to work with the code*/
        ...
        if( line.length( ) > 0 && line.at(0) != '#' ) {
            ...
            if (token == "v" ) {
                ...
                while( lineStream.good() ) {
					...
					// NOTE: static const size_t npos = -1;
                    if( slash1 == string::npos ){		
                        ...
                        pIndex = atoi( vertString.substr(0,slash1).c_str() )
                                        - 1;
						// NOTE: this is for cases without texture coordinates 
						// e.g. 2//25
                        if( slash2 > slash1 + 1) {
                                tcIndex =
                    ...
                    if( pIndex == -1 ) {
                        ...
                        face.push_back(vec2(pIndex,tcIndex));
                    }
					/*
...
void VBOMesh::center( vector<vec3> & points ) {
	...
	cout << maxPoint.x << " " << maxPoint.y << endl;
}
        ...
        const vector<vec2> & faces )
{
    ...
    for( GLuint i = 0; i < faces.size(); i += 3) {
        ...
        const vec3 & p2 = points[faces[i+1].x];
        const vec3 & p3 = points[faces[i+2].x];
        ...
        normals[faces[i].x] = n;
        normals[faces[i+1].x] = n;
        normals[faces[i+2].x] = n;
    }

    for( GLuint i = 0; i < normals.size(); i++ ) {
        normals[i] = glm::normalize(normals[i]);
		//cout << "x: " << normals[i].x << " y:" << normals[i].y << " z: " << normals[i].z << endl;
    }
}
                        ...
                        const vector<vec2> &elements )
{
    ...
    for( GLuint i = 0; i < nVerts; ++i )
    {
        ...
        if( tc != NULL ) {
			tcIdx = elements[i].x;
            tc[tcIdx] = texCoords[elements[i].y].x;
            tc[tcIdx+1] = texCoords[elements[i].y].y;
        }
        if( tang != NULL ) {
