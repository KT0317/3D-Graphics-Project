...
GLuint
GeomShader(const char* vShaderFile, const char* fShaderFile, const char* gShaderFile, const char* outputAttributeName) {
	struct Shader {
		const char*  filename;
		GLenum       type;
		GLchar*      source;
	...
	std::cout << "Linked " << vShaderFile << " " << fShaderFile << " " << gShaderFile << std::endl;
...
GLuint Program::loadShaderPair(std::string vertShader, std::string fragShader ){
    ...
    _prog = InitShader(vertShader.c_str(),fragShader.c_str(), "fragColour");
...
GLuint Program::loadShaderPairWithGeom(std::string vertShader, std::string fragShader, std::string geomShader) {

	_prog = GeomShader(vertShader.c_str(), fragShader.c_str(), geomShader.c_str(), "fragColour");
...
GLuint Shaders::loadShaderPairWithGeom(std::string name)
{
	std::string vertShader = _dir + name + ".vert";
	std::string fragShader = _dir + name + ".frag";
	std::string geomShader = _dir + name + ".geom";


	Program *p = new Program();
	GLuint pid = p->loadShaderPairWithGeom(vertShader, fragShader, geomShader);
